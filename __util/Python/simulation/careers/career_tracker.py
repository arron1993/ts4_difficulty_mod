...
# Compiled at: 2020-10-20 22:33:10
# Size of source mod 2**32: 36325 bytes
import itertools
...
class CareerTracker(AffordanceCacheMixin, SimInfoTracker):
    ...
    def _on_confirmation_dialog_response(self, dialog, new_career, career_level_override=None, schedule_shift_override=CareerShiftType.ALL_DAY, disallowed_reward_types=()):
        if dialog.accepted:
            self.add_career(new_career, career_level_override=career_level_override,
    ...
    def add_career(self, new_career, show_confirmation_dialog=False, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, post_quit_msg=True, schedule_shift_override=CareerShiftType.ALL_DAY, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False):
        if show_confirmation_dialog:
            level, _, track = new_career.get_career_entry_data(career_history=(self._career_history),
            ...
            if self._retirement is not None:
                self._retirement.send_dialog((Career.UNRETIRE_DIALOG), (career_level_tuning.get_title(self._sim_info)),
                  ...
                  on_response=(lambda dialog: self._on_confirmation_dialog_response(dialog, new_career, career_level_override=career_level_override, schedule_shift_override=schedule_shift_override)))
                return
    ...
    def get_quittable_careers(self, schedule_shift_type=CareerShiftType.ALL_DAY):
        quittable_careers = dict(((uid, career) for uid, career in self._careers.items() if career.can_quit if career.get_is_quittable_shift(schedule_shift_type)))
    ...
    def update_history(self, career, from_leave=False):
        ...
        self._career_history[career.guid64] = CareerHistory(career_track=(career.current_track_tuning), level=(career.level),
