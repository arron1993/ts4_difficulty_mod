...
# Compiled at: 2020-10-21 01:31:01
# Size of source mod 2**32: 225858 bytes
...
from careers.career_enums import CareerOutfitGenerationType, CareerShiftType, ReceiveDailyHomeworkHelp, CareerCategory, CareerPanelType, GigResult
...
class CareerBase(CoworkerMixin, CareerKnowledgeMixin, sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    def is_career_available(cls, inst, sim_info=DEFAULT, from_join=False):
        ...
        if not from_join:
            if not inst_or_cls.allow_multiple_careers:
                if any((career.guid64 != inst_or_cls.guid64 and career.career_category != CareerCategory.AdultPartTime and career.career_category == inst_or_cls.career_category for career in sim_info.career_tracker)):
                    return False
    ...
    def is_career_selectable(cls, inst, sim_info=DEFAULT):
        ...
        sim_info = inst_or_cls.sim_info if sim_info is DEFAULT else sim_info
        resolver = SingleSimResolver(sim_info)
        return inst_or_cls.career_selectable_tests.run_tests(resolver)
    ...
    def join_career(self, career_history=None, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, schedule_shift_override=CareerShiftType.ALL_DAY, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False):
        ...
        self._sim_info.career_tracker.update_history(self)
        self.career_start(schedule_init_only=schedule_init_only)
    ...
    def career_start(self, is_load=False, is_level_change=False, schedule_init_only=False):
    ...
    def career_stop(self, for_travel=False, is_level_change=False):
        if self._work_scheduler is not None:
            ...
            if not for_travel:
                self._work_scheduler = None
            elif self._upcoming_gig is not None:
                for_travel or is_level_change or self._clear_current_gig()
        else:
            if self._work_scheduler_override is not None:
    ...
    def early_warning_callback(self):
        if not services.get_career_service().enabled:
            ...
            if self.should_skip_next_shift() or self._should_automatically_use_pto():
                return
            elif self._requested_day_off_reason == CareerTimeOffReason.NO_TIME_OFF:
            ...
            assignments_to_offer = self.get_assignments_to_offer(just_accepted=False)
            self._offered_assignment_ids = set((assignment.guid64 for assignment in assignments_to_offer))
            self._offered_assignment_ids or self.send_career_message(self.career_messages.career_early_warning_notification)
        else:
            if self.requested_day_off_reason == CareerTimeOffReason.WORK_FROM_HOME:
                self._start_work_from_home()
            else:
    ...
    def _start_work_callback(self, scheduler, alarm_data, extra_data):
        if self._at_work:
            ...
            if not services.get_career_service().enabled:
                ...
                if self._upcoming_gig:
                    if self._upcoming_gig.treat_work_time_as_due_date():
    ...
    def _try_offer_career_event(self):
        if services.get_persistence_service().is_save_locked():
            return False
            return self._sim_info.is_npc or self._sim_info.is_instanced() or False
        ...
        services.get_career_service().add_pending_career_event_offer(self, career_event,
          on_accepted=(self._on_career_event_accepted),
          on_canceled=(self._on_career_event_declined))
        return True
    ...
    def handle_career_loot(self, hours_worked, left_early=False):
        if self.on_assignment:
            ...
            if self._upcoming_gig:
                if not left_early or self._upcoming_gig.odd_job_tuning is not None:
    ...
    def populate_set_career_op(self, career_op):
        ...
        if not self.sim_info.is_npc:
            self.populate_set_career_op_with_data_for_played_sims(career_op)

    def populate_set_career_op_with_data_for_played_sims(self, career_op):
        ...
        if self._upcoming_gig is not None:
            self._upcoming_gig.build_gig_msg((career_op.gig_info), (self._sim_info),
              gig_time=(self._upcoming_gig.get_gig_time()),
              gig_customer=(self._upcoming_gig.get_gig_customer()))
        if self.career_panel_type == CareerPanelType.UNIVERSITY_COURSE and self.is_course_slot:
            degree_tracker = self._sim_info.degree_tracker
            if degree_tracker is not None:
 ...
 None, away_action=tone, 
    ...
    def on_zone_load(self):
        ...
        if self._upcoming_gig:
            self._upcoming_gig.on_zone_load()
    ...
    def load_from_persistable_sim_career_proto(self, proto, skip_load=False):
        ...
        if skip_load:
            ...
            if self._pending_promotion and self._level + 1 >= len(self._current_track.career_levels):
                ...
                self._sim_info.has_outfit((OutfitCategory.CAREER, 0)) or self.generate_outfit()
            aspiration_manager = services.get_instance_manager(sims4.resources.Types.ASPIRATION)
    ...
    def _clear_current_gig(self):
        ...
        if self._work_scheduler_override:
            self._work_scheduler_override.destroy()
            self._work_scheduler_override = None
        follow_up_gig, schedule_time, customer = self._get_follow_up_gig_data()
        self._upcoming_gig.clean_up_gig()
        self._upcoming_gig = None
        if follow_up_gig is not None:
            self.set_gig(follow_up_gig, schedule_time, gig_customer=customer)
        self.resend_career_data()

    def _get_follow_up_gig_data(self):
        if not self._upcoming_gig is None:
            if self._upcoming_gig.follow_up_gig is None:
                return (None, None, None)
            gig_result = self._upcoming_gig.gig_result
            if gig_result is None:
                return (None, None, None)
        else:
            if gig_result != GigResult.GREAT_SUCCESS:
                if gig_result != GigResult.SUCCESS:
                    return (None, None, None)
            follow_up_gig = self._upcoming_gig.follow_up_gig
            now = services.time_service().sim_now
            schedule_time = follow_up_gig.get_time_until_next_possible_gig(now)
            if schedule_time is None:
                logger.error("Current gig {} is trying to push follow-up gig {} but can't get a valid schedule time. Follow-up gig won't be pushed.", (self._upcoming_gig), follow_up_gig, owner='trevor')
                return (None, None, None)
            schedule_time = now + schedule_time
            customer = None
            customer_id = self._upcoming_gig.get_gig_customer()
            if customer_id:
                customer = services.sim_info_manager().get(customer_id)
                if customer is None:
                    logger.error("Current gig {} is trying to push follow-up gig {} with customer ID {} but the Sim can't be found.", (self._upcoming_gig), follow_up_gig, customer_id, owner='trevor')
                    return (None, None, None)
        return (
    ...
    def _set_up_gig_timers(self):
        if self._upcoming_gig is None or self._upcoming_gig.gig_time is None:
            return
        elif self.career_messages.career_early_warning_time is not None:
            early_warning_time_span = date_and_time.create_time_span(hours=(self.career_messages.career_early_warning_time))
        else:
            early_warning_time_span = None
        self._work_scheduler_override = self._upcoming_gig.gig_time(schedule_immediate=True, start_callback=(self._start_work_callback),
          early_warning_callback=(self.early_warning_callback),
          early_warning_time_span=early_warning_time_span,
          required_start_time=(self._upcoming_gig.get_gig_time()))
