...
# Compiled at: 2020-04-24 23:18:59
...
class HomeAssignmentGig(Gig):
    ...
    def is_objective_active(self, objective):
        ...
        if self.gig_assignment_aspiration is not None:
            if objective in aspiration_tracker.get_objectives(self.gig_assignment_aspiration):
                return True
        bonus_gig_aspiration = self.get_bonus_gig_aspiration()
        if bonus_gig_aspiration is not None:
            if objective in aspiration_tracker.get_objectives(bonus_gig_aspiration):
                return True
        return False
