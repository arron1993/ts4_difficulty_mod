...
# Compiled at: 2020-06-08 21:31:54
...
class CareerService(Service):
                ...
                if sim_info.is_npc:
                    ...
                    if career.currently_at_work:
                        if career.is_at_active_event:
                            career.career_event_manager.is_valid_zone_id(sim_info.zone_id) or career.end_career_event_without_payout()
                            continue
                        else:
                            if not career._rabbit_hole_id:
                                career.put_sim_in_career_rabbit_hole()
                if career.currently_at_work:
                    sim_info.can_go_to_work(zone_id=(sim_info.zone_id)) or career.leave_work(left_early=True)
    ...
    def _try_offer_next_career_event(self):
        if self._pending_career_events:
            ...
            pending.career.send_career_message((pending.career.career_messages.career_event_confirmation_dialog),
              on_response=(self._on_career_event_response),
