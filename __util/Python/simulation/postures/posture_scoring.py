...
# Compiled at: 2020-10-02 01:01:22
# Size of source mod 2**32: 52294 bytes
...
import accumulator, gsi_handlers, postures.posture_specs, primitives.routing_utils, services, sims4.reload, socials.geometry
        ...
        if reservation_handler is not None:
            ...
            if interaction.score_additional_sims_for_in_use is not None:
                for participant, part_definition in interaction.score_additional_sims_for_in_use.items():
                    ...
                    targets_to_check = {}
                    if target.is_part:
                        targets_to_check = {part for part in target.part_owner.parts if part_definition is part.part_definition}
    ...
    def get_goal_node_cost(goal_node, sim, interaction, var_map, preferences, included_sis, additional_template_dict, relationship_bonuses, spec_constraint, group_constraint, spec_slots_and_costs):
        ...
        if reservation_handler is not None:
            if goal_body_target is not None:
                if not goal_body_target.is_sim:
                    if not may_reserve_posture_target(sim, goal_node, goal_body_target, interaction=interaction):
            ...
            if goal_surface_target is not None and postures.posture_specs.PostureSpecVariable.SLOT in var_map:
                ...
                objects_to_ignore = []
                if hasattr(interaction, 'process') and interaction.process is not None:
        ...
        if goal_body_target is not None:
            ...
            if not interaction.is_putdown:
                ...
                if goal_body_target is not None:
                    if interaction.combined_posture_target_preference is not None:
                        ...
                        if goal_surface_target is not None and goal_surface_target.posture_transition_target_tag != postures.PostureTransitionTargetPreferenceTag.INVALID:
                            ...
                            if goal_body_target is not None and goal_body_target.posture_transition_target_tag != postures.PostureTransitionTargetPreferenceTag.INVALID:
                ...
                if goal_surface_target is not None and interaction.posture_surface_slotted_object_preference is not None:
