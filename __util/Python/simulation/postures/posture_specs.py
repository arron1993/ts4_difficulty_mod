...
# Compiled at: 2020-06-09 23:11:30
# Size of source mod 2**32: 112207 bytes
import cython
...
class PostureOperationBodyTransition(PostureOperationOperationBase):
    ...
    def apply(self, spec: PostureSpec) -> PostureSpec:
        if spec.carry.target is not None:
            ...
            if source_posture_type.mobile and surface_target is None:
                if not self._posture_type.mobile:
                    if dest_target_is_not_none and source_target is not None:
                        if source_target != destination_target:
                            return
                        else:
                            if (source_posture_type.mobile or self._posture_type).mobile:
