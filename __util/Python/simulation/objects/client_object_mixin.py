...
# Compiled at: 2020-06-09 01:21:07
# Size of source mod 2**32: 75169 bytes
...
import animation.arb, assertions, caches, distributor.ops, distributor.sparse, enum, native.animation, objects.definition, routing, services, sims4.log, uid
...
class ClientObjectMixin:
    ...
    def hover_tip(self, value):
        if value is not None:
            ...
            data = (
             ClientObjectMixin.HOVERTIP_HANDLE, 'hover_tip', value)
        ...
        for name, value in ui_metadata.items():
            if name in self._ui_metadata_cache:
                if self._ui_metadata_cache[name] == value:
                    if use_cache:
                        continue
            elif name in self._generic_ui_metadata_setters:
    ...
    def parent(self):
        ...
        if parent is not None:
            ...
            if self not in children:
                return self.is_part and self.part_owner in children or None
        return parent
