...
# Compiled at: 2020-07-21 20:36:00
# Size of source mod 2**32: 62962 bytes
...
from objects.components.state import TunableStateTypeReference, ObjectStateValue
...
class InventoryItemComponent(Component, HasTunableFactory, AutoFactoryInit, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, component_name=types.INVENTORY_ITEM_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.InventoryItemComponent):
    ...
    def _update_stack_id_with_state(self, state):
        custom_key = None
        dirty = False
        if self.stack_scheme_object_state:
            if state is self.stack_scheme_object_state:
                custom_key = self.owner.get_state(state).guid64
                dirty = True
        else:
            if self._stack_scheme_overrides is not None:
                for override_data in self._stack_scheme_overrides:
            ...
            if dirty:
                new_stack_id = services.inventory_manager().get_stack_id(self.owner, self.get_stack_scheme(), custom_key)
    ...
    def refresh_decay_modifiers(self):
        if not self.is_in_inventory():
            ...
            if effects is None:
                return
            test_result = self._run_decay_modifiers_tests(effects)
            if test_result:
                self._is_decay_modifiers_applied or self._apply_decay_modifiers(effects)
        elif not test_result:
            if self._is_decay_modifiers_applied:
                self._remove_decay_modifiers(effects)
    ...
    def _apply_inventory_effects(self, inventory_type):
        ...
        if effects:
            if not self._is_decay_modifiers_applied:
                if self._run_decay_modifiers_tests(effects):
                    self._apply_decay_modifiers(effects)
