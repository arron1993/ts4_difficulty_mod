...
# Compiled at: 2020-08-28 23:08:32
# Size of source mod 2**32: 18741 bytes
...
class ListenSuperInteraction(SuperInteraction):
    ...
    def __init__(self, aop, context, required_station=None, off_state=None, **kwargs):
        ...
        if required_station is not None:
            ...
            if self.required_station is None:
                ...
                if self.off_state is None:
                    self.off_states or logger.error('Listen interaction {} does not have an off state. This must be tuned on the interaction or the stereo component', self, owner='thomaskenney')
    ...
    def _changed_state_callback(self, target, state, old_value, new_value):
        if not (self.off_state and new_value != self.off_state):
            if new_value not in self.off_states:
