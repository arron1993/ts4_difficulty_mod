...
# Compiled at: 2020-10-02 23:45:07
# Size of source mod 2**32: 76959 bytes
...
class TraitTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def __call__(self, test_targets=()):
        ...
        for target in test_targets:
            ...
            if not self.whitelist_traits:
                if self.whitelist_trait_types:
...
class GenderPreferenceTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def _check_gender_preference(self, sim_a, sim_b, overide_target_gender=None):
        ...
        if self.fallback_if_no_perference_is_set:
            if not sim_a.has_gender_preference_been_set():
                if sim_a.gender == sim_b_gender:
                    return TestResult(False, 'GenderPreferenceTest: Sim, {}, has same gender as Sim, {} gender {}, when using fallback.',
                      sim_a,
                      sim_b,
                      sim_b_gender,
            ...
            return GenderPreferenceTest.GENDER_PREFERENCE_THRESHOLD.compare(preference_stat.get_value()) or TestResult(False, "GenderPreferenceTest: Sim, {}, doesn't have proper gender preference to Sim, {} gender {}.", sim_a,
              sim_b,
              sim_b_gender,
              tooltip=(self.tooltip))
        return TestResult.TRUE
            ...
            if not subject_participant.is_sim:
                ...
                for target_participant in target_participants:
                    if not target_participant.is_sim:
                        ...
                        if not result:
                            return result
                        result = self._ignore_reciprocal or self._check_gender_preference(target_participant, subject_participant)
...
class FilterTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'filter_target':OptionalTunable(tunable=TunableEnumEntry(description='\n                The sim that will have the filter checked against.\n                ',
     ...
     'relative_sim':TunableEnumEntry(description='\n            The sim that will be the relative sim that the filter will\n            check against for relative checks such as relationships or\n            household ids.\n            ',
       tunable_type=ParticipantType,
       default=ParticipantType.Actor), 
     'sim_filter':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.SIM_FILTER)), 
                ...
                if filter_targets is not None:
                    for filter_target in filter_targets:
                        for relative_sim_info in relative_sims:
                            ...
                            if self.threshold_matched is not None:
                                ...
                                return matched or TestResult(False, 'FilterTest: Sim {} (id {}) does not match filter {}.', (filter_target.full_name),
                    ...
                    for relative_sim_info in relative_sims:
                        ...
                        if self.threshold_matched is not None:
                            ...
                            return results or TestResult(False, 'FilterTest: Sim {} (id {}) does not match filter {}.', (relative_sim_info.full_name),
...
class SimInfoGameplayOptionsTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
 ...
 L.1616        74  LOAD_GLOBAL              TestResult
               76  LOAD_ATTR                TRUE
               78  RETURN_VALUE     
             80_0  COME_FROM            72  '72'
             80_1  COME_FROM            68  '68'

 L.1618        80  LOAD_GLOBAL              TestResult
               82  LOAD_CONST               False
