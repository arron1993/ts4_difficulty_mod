...
# Compiled at: 2020-08-03 19:47:08
# Size of source mod 2**32: 44755 bytes
...
class VenueService(Service):
 ...
 L. 262         6  LOAD_CONST               None
                8  RETURN_VALUE     
             10_0  COME_FROM             4  '4'

 L. 264        10  LOAD_FAST                'self'
 ...
 L. 278       120  LOAD_FAST                'force_start_situations'
              122  POP_JUMP_IF_FALSE   134  'to 134'

 L. 279       124  LOAD_FAST                'self'
 ...
 L. 281       134  LOAD_FAST                'type_changed'
              136  RETURN_VALUE     
               -1  RETURN_LAST      
    ...
    def is_zone_valid_for_venue_type(self, zone_id, venue_types, compatible_region=None, ignore_region_compatability_tags=False, region_blacklist=[]):
        if not zone_id:
            ...
            if compatible_region is not None:
                ...
                return venue_region is None or compatible_region.is_region_compatible(venue_region, ignore_tags=ignore_region_compatability_tags) or False
            if region_blacklist:
    ...
    def get_zones_for_venue_type_gen(self, *venue_types, compatible_region=None, ignore_region_compatability_tags=False, region_blacklist=[]):
        for neighborhood_proto in services.get_persistence_service().get_neighborhoods_proto_buf_gen():
            for lot_owner_info in neighborhood_proto.lots:
                ...
                if self.is_zone_valid_for_venue_type(zone_id, venue_types,
    ...
    def load(self, zone_data=None):
        ...
        for zone_data_msg in zones:
            ...
            if source_venue_type is None:
                self.set_provider(zone_data_msg.zone_id, None)
                continue
            if source_venue_type.variable_venues is None:
                self.set_provider(zone_data_msg.zone_id, None)
                continue
