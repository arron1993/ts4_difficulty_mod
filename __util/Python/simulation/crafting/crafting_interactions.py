...
# Compiled at: 2020-09-15 20:45:04
# Size of source mod 2**32: 158815 bytes
...
def create_craftable(chosen_recipe, crafter_sim, inventory_owner=None, quality=None, owning_household_id_override=None, place_in_inventory=False, post_add=None, **kwargs):
    ...
    def _handle_begin_crafting(self, recipe, crafter, ordering_sim=None, crafting_target=None, orderer_ids=DEFAULT, ingredients=(), funds_source=None, paying_sim=None):
        if orderer_ids is DEFAULT:
            ...
            if hasattr(self, 'ingredient_source'):
                ...
                elif recipe.use_ingredients and ingredients is not None:
...
class StartCraftingSuperInteraction(StartCraftingMixin, PickerSuperInteraction):
    ...
    def picker_rows_gen(cls, inst, target, context, crafter=DEFAULT, order_count=1, recipe_ingredients_map=None, funds_source=None, **kwargs):
        ...
        for recipe in recipe_list:
            ...
            if subclass_of_order_interaction:
                if not (cls.ingredient_source and has_required_ingredients):
            ...
            recipe_test_result = CraftingProcess.recipe_test(target, context, recipe, crafter, discounted_price, paying_sim=(context.sim),
              funds_source=funds_source,
            ...
            if recipe_test_result.visible:
                ...
                if recipe_test_result.errors:
                        ...
                        if recipe.all_ingredients_required:
                            ...
                            if not has_required_ingredients:
                                if crafter is not context.sim:
                                    continue
                                if recipe.use_ingredients.missing_ingredient_tooltip_style == IngredientTooltipStyle.DEFAULT_MISSING_INGREDIENTS:
                    ...
                    elif recipe.has_final_product_definition:
                    ...
                    if stored_cas_parts:
                        ...
                        if all((household.part_in_reward_inventory(stored_cas_parts) for stored_cas_parts in stored_cas_parts)):
                            locked_in_cas_icon = cls.CAS_UNLOCKED_ICON
                        else:
                            locked_in_cas_icon = cls.CAS_LOCKED_ICON
                    row = RecipePickerRow(name=(recipe.get_recipe_name(crafter)), price=original_price,
                      ...
                      tag=recipe,
                      ingredients=ingredient_display_list,
                      price_with_ingredients=discounted_price,
                      ...
                      locked_in_cas_icon=locked_in_cas_icon,
...
class StartCraftingAutonomouslySuperInteraction(StartCraftingMixin, AutonomousPickerSuperInteraction):
    ...
    def _run_interaction_gen(self, timeline):
        ...
        if recipe is None:
            return False
        return self._handle_begin_crafting(recipe, (self.sim), orderer_ids=(self.orderer_ids), funds_source=(self.funds_source))
        if False:
            yield None
...
class StartCraftingOrderAutonomouslySuperInteraction(StartCraftingAutonomouslySuperInteraction):
    INSTANCE_TUNABLES = {'crafter':TunableEnumEntry(ParticipantType, ParticipantType.Object, description='Who or what to apply this test to'), 
     'order_craft_affordance':TunableReference(services.affordance_manager(), description='The affordance used to order the chosen craft'), 
     'order_wait_affordance':TunableReference(services.affordance_manager(), description='The affordance used to wait for an ordered craft'), 
...
class CraftingPhaseSuperInteractionMixin(CraftingInteractionMixin):
    INSTANCE_TUNABLES = {'crafting_type_requirement':TunableReference(services.recipe_manager(), class_restrictions=CraftingObjectType,
       allow_none=True,
