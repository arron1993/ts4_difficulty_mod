...
# Compiled at: 2020-06-23 18:10:55
# Size of source mod 2**32: 19533 bytes
...
def show_lot_score(show_load_time: bool=True, use_raw_score=False, _connection=None):
    ...
    if use_raw_score:
        sims4.commands.cheat_output('Total Lot Score = {}'.format(total_lot_score), _connection)
    else:
        sims4.commands.cheat_output('Total Lot Score = {:.02f}%'.format(total_lot_score), _connection)
    if show_load_time:
        ...
        if show_load_time:
            ...
            sims4.commands.cheat_output('Total Lot Load Time = {}'.format(lot_load_time), _connection)
