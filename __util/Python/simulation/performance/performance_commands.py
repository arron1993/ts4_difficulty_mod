...
# Compiled at: 2020-07-14 20:12:56
# Size of source mod 2**32: 54578 bytes
...
def dump_tests_profile(sort: SortStyle=SortStyle.ALL, _connection=None):
    ...
    if event_testing.resolver.test_profile is None:
        ...
        def test_callback(file):
            ...
            for test_name, test_metrics in sorted((event_testing.resolver.test_profile.items()), key=(lambda t: get_sort_style(t[1].metrics)),
              reverse=True):
...
def get_relationship_metrics(output=None):
    ...
    for relationship in rel_service:
        ...
        rel_tracks += len(relationship.sentiment_track_tracker(y_id))
        rel_tracks += len(relationship.sentiment_track_tracker(x_id))
        rels += 1
