...
# Compiled at: 2016-02-11 09:04:25
...
def generate_story_progression_view_data(sim_id: int=None):
    ...
    for story_progression_action in sim_info.story_progression_tracker.get_actions_gen():
        data.append({'action_id':story_progression_action.action_id, 
         'action_name':str(story_progression_action), 
         'action_duration':str(story_progression_action.get_duration())})
...
def archive_sim_story_progression(sim_info, action, *, result, global_demographics=(), action_demographics=()):
    archive_data = {'action_name':str(action), 
     'action_result':str(result), 
     'action_duration':str(action.get_duration()), 
     'action_demographics':[]}
    for global_demographic, action_demographic in zip(global_demographics, action_demographics):
        archive_data['action_demographics'].append({'demographic_name':str(action_demographic), 
         'demographic_previous_error':str(global_demographic.get_demographic_error()), 
         'demographic_current_error':str(action_demographic.get_demographic_error())})
...
def archive_story_progression(action, message, *args):
    entry = {'action_type':str(action), 
     'action_message':message.format(*args)}
