...
# Compiled at: 2020-02-27 00:53:48
# Size of source mod 2**32: 53682 bytes
from _functools import reduce
import collections, game_services, operator
...
import alarms, enum, event_testing, services, sims4.log, sims4.random, telemetry_helper, uid
...
class WhimsTracker(SimInfoTracker):
    ...
    def whims_and_parents_gen(self):
        for whim_data in self._active_whims:
            if whim_data.whim is None:
                continue
            yield (
             whim_data.whim, whim_data.whimset)
    ...
    def get_active_whimsets(self):
        ...
        if open_street_director is not None:
            if open_street_director.whim_set:
    ...
    def _get_currently_active_whim_types(self):
        return {type(whim_data.whim) for whim_data in self._active_whims if whim_data.whim is not None if whim_data.whim is not None}

    def _get_currently_used_whimsets(self):
        return {whim_data.whimset for whim_data in self._active_whims if whim_data.whimset is not None if whim_data.whimset is not None}
    ...
    def _on_goal_completed(self, whim, whim_completed):
        if not whim_completed:
            ...
            for set_to_chain in parent_whimset.connected_whim_sets:
                ...
                self._activate_whimset(set_to_chain, target=inherited_target_sim_info,
                  chained=True)
            ...
            if highest_chained_priority > 0:
                thrashed = self._try_and_thrash_whims(highest_chained_priority, extra_prohibited_whims={
                 whim_type})
            thrashed or self._offer_whims(prohibited_whims={whim_type})
    ...
    def _create_whim(self, whimset, prohibited_whims):
        ...
        if potential_target is None:
            if whimset.force_target is not None:
                return
        elif whimset.secondary_target is not None:
        ...
        while weighted_whims:
            ...
            if old_whim_instance_and_whimset is not None:
                if old_whim_instance_and_whimset[0].is_on_cooldown():
                    continue
            pretest = selected_whim.can_be_given_as_goal(sim, None, inherited_target_sim_info=potential_target)
            if pretest:
                whim = selected_whim(sim_info=(self._sim_info), goal_id=(self._goal_id_generator()),
    ...
    def _offer_whims(self, prohibited_whimsets=EMPTY_SET, prohibited_whims=EMPTY_SET):
        ...
        for index, whim_data in enumerate(self._active_whims):
            if whim_data.whim is not None:
                continue
            elif index == self.emotional_whim_index:
    ...
    def _send_goals_update(self):
        if not self._goals_dirty:
            return
        logger.debug('Sending whims update for {}.  Current active whims: {}', (self._sim_info),
          (self._active_whims),
          owner='jjacobson')
        current_whims = []
        for index, whim_data in enumerate(self._active_whims):
