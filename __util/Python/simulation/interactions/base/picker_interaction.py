...
# Compiled at: 2020-10-20 02:49:02
# Size of source mod 2**32: 173417 bytes
...
from sims4.tuning.tunable import TunableEnumEntry, OptionalTunable, TunableVariant, Tunable, TunableTuple, TunableReference, TunableSet, TunableList, TunableFactory, TunableThreshold, TunableMapping, TunableRange, HasTunableSingletonFactory, AutoFactoryInit, TunableEnumWithFilter
    ...
    def _push_continuations(self, sim_ids, zone_datas=None):
        ...
        if self.link_continuation != SimPickerLinkContinuation.NEITHER:
            if actor_continuation is not None:
                if target_continuation is not None and not self.link_continuation == SimPickerLinkContinuation.TARGET:
                ...
                for interaction in picked_continuations:
                    if not self.link_continuation == SimPickerLinkContinuation.ACTOR:
                        if self.link_continuation == SimPickerLinkContinuation.ALL:
                            interaction.attach_interaction(actor_continuation)
                        if self.link_continuation == SimPickerLinkContinuation.PICKED or self.link_continuation == SimPickerLinkContinuation.ALL:
                            pass
                        actor_continuation.attach_interaction(interaction)

            if target_continuation is not None:
                if actor_continuation is not None and not self.link_continuation == SimPickerLinkContinuation.ACTOR:
                    if self.link_continuation == SimPickerLinkContinuation.ALL:
                        target_continuation.attach_interaction(actor_continuation)
                for interaction in picked_continuations:
                    if not self.link_continuation == SimPickerLinkContinuation.TARGET:
                        if self.link_continuation == SimPickerLinkContinuation.ALL:
...
class SimPickerInteraction(SimPickerMixin, PickerSingleChoiceSuperInteraction):
    ...
    def get_single_choice_and_row(cls, inst, context=None, target=None, **kwargs):
        ...
        if single_choice is not None:
            return (
             single_choice.sim_info,
             inst_or_cls.create_row(single_choice.sim_info.sim_id))
        return (None, None)
...
class ObjectInInventoryPickerMixin:
    ...
    def _setup_dialog(self, dialog, **kwargs):
        if self.suppress_duplicate_objects is not None:
            subject = (self.get_participant)(participant_type=self.inventory_subject, sim=self.context.sim, 
...
class PurchasePickerInteraction(PickerSuperInteraction):
    ...
    def _setup_dialog(self, dialog, **kwargs):
        ...
        self._setup_delivery_method(purchase_picker_data)
        self._populate_items(purchase_picker_data)
        ...
        dialog.use_dialog_pick_response = self._supports_pick_response()
        right_custom_text = self._get_right_custom_text()
        ...
        for (definition, custom_price), objects in purchase_picker_data.items_to_purchase.items():
            if multiplier != 1:
                ...
                tooltip = None
            row = PurchasePickerRow(def_id=(definition.id), is_enable=(self._get_enabled_option(definition)),
              num_owned=(self._get_count_option(definition)),
              num_available=(self._get_availability_option(definition)),
              tags=(definition.build_buy_tags),
              custom_price=custom_price,
              objects=objects,
              row_tooltip=tooltip,
              show_discount=(self.show_discount),
              icon_override=(self._get_icon_override_option(definition)),
              row_description=(self._get_description_override_option(definition)))
            dialog.add_row(row)
...
class ObjectsInMultipleInventoriesPurchasePickerInteraction(ObjectsInMultipleInventoriesMixin, PurchasePickerInteraction):
    ...
    @classmethod
    def has_valid_choice(cls, target, context, **kwargs):
        for purchase_method in (cls._get_objects_internal_gen)(target, context, **kwargs):
            return True
    ...
    def _add_picked_items(self, picked_items, definition, total_count):
        ...
        if self.use_sim_inventory is not None:
            ...
            while items_added < total_count:
                object_to_remove = inventory_subject.inventory_component.get_item_with_definition(definition)
                if inventory_subject.inventory_component.try_remove_object_by_id(object_to_remove.id):
...
class PurchasePickerWithFailureOptionInteraction(PurchasePickerInteraction):
    ...
    def _build_outcome_sequence(self):
        sequence = self.purchase_outcome.build_elements(self, update_global_outcome_result=True)
        if self.target is not None:
            target_outcome = self.target.get_affordance_outcome(self)
            if target_outcome is not None:
                sequence = (
                 sequence, target_outcome.build_elements(self))
        return sequence