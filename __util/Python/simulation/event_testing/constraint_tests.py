...
# Compiled at: 2019-09-27 23:21:59
    ...
    def __call__(self, constraint_targets=(), test_actors=()):
        ...
        for target in constraint_targets:
            ...
            for sim in instanced_sims:
                if total_constraint.geometry.test_transform(sim.transform):
                    if total_constraint.is_routing_surface_valid(sim.routing_surface):
                        if total_constraint.is_location_water_depth_valid(sim.location) and total_constraint.is_location_terrain_tags_valid(sim.location):
                            if self.test_set(DoubleSimResolver(test_actor, sim.sim_info)) or self.must_be_line_of_sight and not sim.sim_info is test_actor:
