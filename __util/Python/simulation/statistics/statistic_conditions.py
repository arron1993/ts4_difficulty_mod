...
# Compiled at: 2020-10-21 01:33:36
# Size of source mod 2**32: 79917 bytes
...
class TunableStatisticCondition(TunableFactory):
    ...
    def __init__(self, description='Apply an operation to a statistic.', **kwargs):
        (super().__init__)(who=TunableEnumEntry(ParticipantType, (ParticipantType.Actor), description='Who or what to apply this test to'), stat=TunableReference((services.statistic_manager()), description='The commodity we are gaining.'), 
...
class HiddenOrShownCondition(HasTunableFactory, AutoFactoryInit, Condition):
 ...
 L.1726         0  LOAD_FAST                'callback'
                2  LOAD_FAST                'self'
                4  STORE_ATTR               si_callback

 L.1727         6  LOAD_FAST                'owner'
                8  LOAD_FAST                'self'
               10  STORE_ATTR               _owner

 L.1729        12  LOAD_FAST                'self'
               14  LOAD_ATTR                _owner
