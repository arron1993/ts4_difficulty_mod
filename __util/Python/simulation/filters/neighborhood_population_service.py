...
# Compiled at: 2019-10-18 02:10:25
# Size of source mod 2**32: 33146 bytes
...
class _BasePopulationRequest:
    ...
    def _get_region_household_population_data_and_neighborhood_proto(self):
        ...
        return (
         NeighborhoodPopulationService.REGION_TO_HOUSEHOLD_POPULATION_DATA.get(region_id), neighborhood_proto)
...
class _FillZonePopulationRequest(_BasePopulationRequest):
    ...
    def _add_household_template_to_zone(self, household_templates, total_beds, lot_has_double_beds, lot_has_kid_beds, neighborhood_proto, zone_id):
        ...
        if households:
            ...
            self._create_household_from_template_and_add_to_zone(household_template, neighborhood_proto,
              zone_id,
              creation_source='neigh_pop_service: residents')
            return True
        return False
    ...
    def _get_household_templates_and_bed_data(self, zone_id, household_population_data):
        ...
        return (
         household_templates, total_beds, lot_has_double_bed, lot_has_kid_bed)
    ...
    def process_request_gen(self, timeline):
        ...
        if not household_population_data:
            if not self._try_existing_households:
...
class NeighborhoodPopulationService(Service):
    ...
    def add_population_request(self, num_to_fill, neighborhood_id, completion_callback, available_zone_ids, try_existing_households):
        ...
        if account is None:
            return False
        request = _FillZonePopulationRequest(account, num_to_fill,
          neighborhood_id,
          completion_callback,
          available_zone_ids=available_zone_ids,
          try_existing_households=try_existing_households)
