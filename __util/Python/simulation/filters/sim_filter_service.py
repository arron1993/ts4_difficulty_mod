...
# Compiled at: 2020-09-25 22:32:39
# Size of source mod 2**32: 52514 bytes
import collections
...
class _SimFilterRequest(_BaseSimFilterRequest):
    ...
    def _run_filter_query(self):
        ...
        for result in tuple(results):
            if result.sim_info.id in self._blacklist_sim_ids or result.sim_info.id in global_blacklist:
...
class _MatchingFilterRequest(_SimFilterRequest):
    ...
    def _select_sims_from_results(self, results, sims_to_spawn):
        ...
        global_blacklist = services.sim_filter_service().get_global_blacklist()
        for result in tuple(results):
            if self._allow_instanced_sims or result.sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS) or result.sim_info.id in self._blacklist_sim_ids or result.sim_info.id in global_blacklist:
                results.remove(result)
        ...
        if self._sim_filter.use_weighted_random:
            ...
            if len(self._filter_results) < sims_to_spawn and randomization_group:
...
class SimFilterService(Service):
    ...
    def submit_matching_filter(self, number_of_sims_to_find=1, sim_filter=None, callback=None, callback_event_data=None, sim_constraints=None, requesting_sim_info=None, blacklist_sim_ids=EMPTY_SET, continue_if_constraints_fail=False, conform_if_constraints_fail=False, allow_yielding=True, start_time=None, end_time=None, household_id=None, zone_id=None, club=None, allow_instanced_sims=False, additional_filter_terms=(), gsi_source_fn=None):
        request = None
