...
# Compiled at: 2020-10-05 22:54:34
# Size of source mod 2**32: 170829 bytes
...
class CareerFilterTerm(InvertibleFilterTerm):
    ...
    class _CareerUserLevelFilter(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def career_level_override(self):
            pass
    ...
    class _CareerLevelReferenceFilter(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def career_level_override(self):
            if self.career_level_references:
                return random.choice(self.career_level_references)
    ...
    def calculate_score(self, sim_info, **kwargs):
        ...
        if career is not None:
            ...
            if self.career_level is not None:
                ...
                if score == 0:
                    reason = reason or 'Career level does not match actual career level.'
        else:
            score = 0
    ...
    def conform_sim_info_to_filter_term(self, created_sim_info, **kwargs):
        ...
        if career is not None:
            ...
            if self.invert_score:
                ...
                if career.is_valid_career(created_sim_info):
                    ...
                    if self.career_location is not None:
                        ...
                        if self.career_level is not None:
                            career_tracker.add_career(career, user_level_override=(self.career_level.user_level_override()),
                              career_level_override=(self.career_level.career_level_override()))
                        else:
...
class UniversityMajorFilterTerm(InvertibleFilterTerm):
    ...
    def get_random_university(self):
        all_universities = services.get_instance_manager(sims4.resources.Types.UNIVERSITY).get_ordered_types()
    ...
    def _sim_matches_composition_requirements(self, sim_info):
        if self.age is not None:
            ...
            if self.gender is not None:
                if not self.gender.calculate_score(sim_info):
                    return False
            if self.species is not None:
                return self.species.calculate_score(sim_info) or False
        return True
...
class LivesTogetherFilterTerm(InvertibleFilterTerm):
 ...
 L.2037         0  LOAD_FAST                'requesting_sim_info'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_FALSE    20  'to 20'

 L.2038         8  LOAD_GLOBAL              FilterResult
               10  LOAD_CONST               0

 L.2039        12  LOAD_FAST                'sim_info'
               14  LOAD_CONST               ('score', 'sim_info')
               16  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               18  RETURN_VALUE     
             20_0  COME_FROM             6  '6'

 L.2041        20  LOAD_FAST                'self'
 ...
 L.2046        86  LOAD_FAST                'requesting_sim_info'
 ...
 L.2047       108  LOAD_FAST                'home_zone_id_a'
              110  POP_JUMP_IF_FALSE   120  'to 120'

 L.2048       112  LOAD_FAST                'home_zone_id_a'
 ...
 L.2050       148  LOAD_FAST                'in_household'
 ...
 L.2052       172  LOAD_GLOBAL              FilterResult
              174  LOAD_FAST                'self'
...
class GenderPreferenceFilterTerm(BaseFilterTerm):
    ...
    def calculate_score(self, sim_info, requesting_sim_info=None, **kwargs):
        if requesting_sim_info is None:
            return FilterResult('Trying to check gender preference but there is no requesting sim info.', score=0,
              sim_info=sim_info)
        else:
            if not requesting_sim_info.has_gender_preference_been_set():
                if self.fallback_if_no_preference_is_set:
                    if requesting_sim_info.gender != sim_info.gender:
                        return FilterResult(score=1, sim_info=sim_info)
                return FilterResult('Sim has no gender preference set.', score=0,
                  sim_info=sim_info)
            preference_stat = requesting_sim_info.get_gender_preference(sim_info.gender)
            return GenderPreferenceTest.GENDER_PREFERENCE_THRESHOLD.compare(preference_stat.get_value()) or FilterResult('Gender preference filter failed.', score=0,
              sim_info=sim_info)
        return FilterResult(score=1, sim_info=sim_info)
...
class TunableSimFilter(HasTunableReference, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SIM_FILTER)):
    ...
    def create_sim_info(cls, inst, zone_id, blacklist_sim_ids=(), additional_filter_terms=(), sim_constraints=(), **kwargs):
        ...
        for filter_term in filter_terms:
            ...
            if not result:
                filter_term.conform_optional or logger.error('Failed to conform {} to filter term {}. Sim Filter: {}, Reason: {}', created_sim_info, filter_term, cls.__name__, result)
                return result
