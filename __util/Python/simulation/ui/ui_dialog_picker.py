...
# Compiled at: 2020-07-25 01:17:53
# Size of source mod 2**32: 69300 bytes
import math
...
class RecipePickerRow(BasePickerRow):
    ...
    def __init__(self, price=0, skill_level=0, linked_recipe=None, display_name=DEFAULT, ingredients=None, price_with_ingredients=0, mtx_id=None, discounted_price=0, is_discounted=False, bucks_costs=None, locked_in_cas_icon=None, subrow_sort_id=0, **kwargs):
...
class PurchasePickerRow(BasePickerRow):
    ...
    def __init__(self, def_id=0, num_owned=0, tags=(), num_available=None, custom_price=None, objects=EMPTY_SET, show_discount=False, icon_override=None, **kwargs):
        (super().__init__)(**kwargs)
        self.def_id = def_id
    ...
    def populate_protocol_buffer(self, purchase_row_data):
        ...
        if self.icon_override:
            icon_info = IconInfoData(icon_resource=(self.icon_override))
...
class UiDialogObjectPicker(UiDialogOkCancel):
    ...
    class _MaxSelectableAllButOne(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def get_max_selectable(self, picker, resolver):
            return max(0, len(picker.picker_rows) - 1)
    ...
    def pick_definitions_and_counts(self, picked_definitions=[], picked_counts=[], ingredient_check=None):
        if len(picked_definitions) != len(picked_counts):
            logger.error('Picked definitions and picked counts are not the same length', self)
            return False
...
class UiRecipePicker(UiDialogObjectPicker):
    ...
    def _build_row_links(row1, row2):
        if row1.linked_recipe is not None:
            if row1.linked_recipe is row2.tag:
        ...
        if num_columns > 1:
            if picker_type != ObjectPickerType.OBJECT_LARGE:
                if picker_type != ObjectPickerType.OBJECT_SQUARE:
                    logger.error('Tuning error for {}: Only OBJECT_LARGE and OBJECT_SQUARE picker types can have more than 1 column.', instance_class,
                      owner='yozhang')
