...
# Compiled at: 2020-01-31 00:04:36
# Size of source mod 2**32: 8830 bytes
...
import alarms, persistence_error_types, services
...
class UiDialogService(Service):
    ...
    def dialog_pick_result(self, dialog_id: int, picked_results=[], ingredient_check=None) -> bool:
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None:
            if dialog.pick_results(picked_results, ingredient_check):
                return True
        return False
    ...
    def dialog_pick_result_def_and_counts(self, dialog_id: int, picked_def_ids=[], picked_counts=[], ingredient_check=None) -> bool:
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None:
            if dialog.pick_definitions_and_counts(picked_def_ids, picked_counts, ingredient_check):
                return True
        return False
