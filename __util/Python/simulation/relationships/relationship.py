...
# Compiled at: 2020-10-29 22:52:56
# Size of source mod 2**32: 61110 bytes
import itertools
...
class Relationship:
    __slots__ = ('_sim_id_a', '_sim_id_b', '_bi_directional_relationship_data', '_sim_a_relationship_data',
                 '_sim_b_relationship_data', '_level_change_watcher_id', '_culling_alarm_handle',
                 '_last_update_time', '_relationship_multipliers', '__weakref__',
                 '_is_object_rel', '_target_object_id', '_target_object_manager_id',
                 '_target_object_instance_id', '_object_relationship_name')
    ...
    def get_highest_priority_track_bit(self, sim_id):
        return max(self._bi_directional_relationship_data.get_highest_priority_track_bit(), self._get_uni_directional_rel_data(sim_id).get_highest_priority_track_bit())
    ...
    def add_relationship_bit(self, actor_sim_id, target_sim_id, bit_to_add, notify_client=True, pending_bits=EMPTY_SET, force_add=False, from_load=False, send_rel_change_event=True):
        ...
        self._get_uni_directional_rel_data(actor_sim_id).track_tracker.on_relationship_bit_added(bit_to_add, actor_sim_id)
    ...
    def remove_bit(self, actor_sim_id, target_sim_id, bit, notify_client=True, send_rel_change_event=True):
        if bit is None:
            ...
            if send_rel_change_event:
                ...
                if bit.directionality == RelationshipDirection.BIDIRECTIONAL:
                    self._send_relationship_prechange_event(target_sim_info, actor_sim_info)
            elif bit.directionality == RelationshipDirection.BIDIRECTIONAL:
                rel_data = self._bi_directional_relationship_data
            else:
                rel_data = self._get_uni_directional_rel_data(actor_sim_id)
            rel_data.remove_bit(bit)
            self._bi_directional_relationship_data.track_tracker.on_relationship_bit_removed(bit, actor_sim_id)
            self._get_uni_directional_rel_data(actor_sim_id).track_tracker.on_relationship_bit_removed(bit, actor_sim_id)
            if notify_client is True:
                self._notify_client()
        elif not self._is_object_rel:
            if send_rel_change_event:
                self._send_relationship_changed_event(actor_sim_info, target_sim_info)
                if bit.directionality == RelationshipDirection.BIDIRECTIONAL:
                    self._send_relationship_changed_event(target_sim_info, actor_sim_info)
    ...
    def build_printable_string_of_tracks(self):
        ...
        for track in self._bi_directional_relationship_data.track_tracker:
            ret += '\t\t{} = {}; decaying? {}; decay rate: {}; is sentiment?: n\n'.format(track, track.get_value(), track.decay_enabled, track.get_decay_rate())

        for track in self._sim_a_relationship_data.track_tracker:
            ret += '\t\t{} = {}; decaying? {}; decay rate: {}; is sentiment : y; from sim_id:{} to sim_id:{}\n'.format(track, track.get_value(), track.decay_enabled, track.get_decay_rate(), self.sim_id_a, self.sim_id_b)

        for track in self._sim_b_relationship_data.track_tracker:
            ret += '\t\t{} = {}; decaying? {}; decay rate: {}; is sentiment : y; from sim_id:{} to sim_id:{}\n'.format(track, track.get_value(), track.decay_enabled, track.get_decay_rate(), self.sim_id_b, self.sim_id_a)
