...
# Compiled at: 2018-01-19 19:49:59
# Size of source mod 2**32: 6241 bytes
...
class TunableBuffReference(TunableSingletonFactory):
    ...
    def __init__(self, reload_dependent=False, pack_safe=False, allow_none=False, **kwargs):
        (super().__init__)(buff_type=TunableReference(manager=(services.buff_manager()), description='Buff that will get added to sim.',
...
class TunablePackSafeBuffReference(TunableSingletonFactory):
    ...
    def __init__(self, reload_dependent=False, allow_none=False, **kwargs):
        (super().__init__)(buff_type=TunablePackSafeReference(manager=(services.buff_manager()), description='Buff that will get added to sim.',
  allow_none=allow_none,
  reload_dependent=reload_dependent), 
         buff_reason=OptionalTunable(description='\n                            If set, specify a reason why the buff was added.\n                            ',
  tunable=TunableLocalizedString(description='\n                                The reason the buff was added. This will be displayed in the\n                                buff tooltip.\n                                ')), **kwargs)
