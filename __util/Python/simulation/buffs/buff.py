...
# Compiled at: 2020-10-21 19:43:41
# Size of source mod 2**32: 68776 bytes
...
import enum, event_testing.tests, interactions.base.mixer_interaction, services, sims4.log, sims4.resources, statistics.commodity, statistics.static_commodity, tag, topics.topic
...
class Buff(HasTunableReference, RouteEventProviderMixin, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, MixerActorMixin, MixerProviderMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.BUFF)):
    INSTANCE_TUNABLES = {'buff_name':TunableLocalizedStringFactory(description='\n        Name of buff.\n        ',
     ...
     'topics':TunableList(description='\n        Topics that should be added to sim when buff is added.\n        ',
       tunable=TunableReference(manager=(services.topic_manager()),
     ...
     'static_commodity_to_add':TunableSet(description='\n        Static commodity that is added to the sim when buff is added to sim.\n        ',
       tunable=TunableReference(manager=(services.static_commodity_manager()),
     ...
     'additional_posture_costs':TunableMapping(description='\n        For any posture in this mapping, its cost is modified by the\n        specified value for the purpose of transitioning for this\n        interaction.\n        \n        The values tuned here will be added to the values tuned on the \n        interaction as well as other autonomy modifiers on the Sim.\n        \n        For example, Sit is a generally cheap posture. However, some\n        interactions (such as Nap), would want to penalize Sit in favor\n        of something more targeted (such as the two-seated version of\n        nap).\n        ',
       key_type=TunableReference(manager=(services.posture_manager())),
    ...
    def provide_route_events(self, route_event_context, sim, path, failed_types=None, **kwargs):
        ...
        for route_event_cls in self.route_events.single_events:
            if not route_event_cls is not None or failed_types is None or route_event_cls not in failed_types:
                if route_event_context.route_event_already_scheduled(route_event_cls) or route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_cls.test(resolver):
        ...
        for route_event_cls in self.route_events.repeating_events:
            if not route_event_cls is not None or failed_types is None or route_event_cls not in failed_types:
                if route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_cls.test(resolver):
        ...
        for route_event_cls in self.route_events.last_indoor:
            if route_event_cls is not None:
                if failed_types is None or route_event_cls not in failed_types:
                    pass
                if route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_cls.test(resolver):
