...
# Compiled at: 2020-08-06 21:22:33
# Size of source mod 2**32: 32783 bytes
...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, TunableRange, TunableMapping, TunableList, TunableTuple, TunableEnumFlags, OptionalTunable, TunableReference, TunableEnumEntry
...
class WalksStyleBehavior(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_walkstyle_for_path(self, actor, path, gsi_archiver=None):
        ...
        if actor.is_sim:
            if actor.in_pool:
                if walkstyle not in self.SWIMMING_WALKSTYLES:
                ...
                if posture.mobile:
                    if posture.compatible_walkstyles and walkstyle not in posture.compatible_walkstyles:
