...
# Compiled at: 2020-04-30 02:25:58
...
from routing.object_routing.object_route_variants import ObjectRoutingBehaviorFromWaypointGenerator, ObjectRoutingBehaviorFromRoutingSlotConstraint, ObjectRouteFromRoutingFormation, ObjectRouteFromFGL, ObjectRouteFromTargetObject, ObjectRouteFromParticipantType, ObjectRoutingBehaviorFromLocator
...
class ObjectRoutingBehavior(HasTunableReference, SubclassableGeneratorElement, metaclass=HashedTunedInstanceMetaclass, manager=services.snippet_manager()):
    INSTANCE_TUNABLES = {'route':TunableVariant(description='\n            Define how this object routes when this behavior is active.\n            ',
       ...
       from_target_object=ObjectRouteFromTargetObject.TunableFactory(locked_args={'route_fail': None}),
    ...
    def _do_single_route_gen(self, timeline, route):
        if not route:
            ...
            if not result:
                ...
                yield from nodes and nodes.plan_success or self._route_data.do_route_fail_gen(timeline)
                return True
            if self._canceled:
                return False
            ...
            if self.clear_locomotion_mask:
                mask_override = 0
                track_override = 9999
            self._element = follow_path_element = FollowPath((self._obj), (plan_primitive.path), track_override=track_override,
