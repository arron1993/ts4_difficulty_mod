...
# Compiled at: 2020-08-19 23:19:56
...
class SituationGuestList:
    ...
    def get_other_travelers(self, traveling_sim):
        ...
        for guest_infos in self._job_type_to_guest_infos.values():
            for guest_info in guest_infos:
                if guest_info.sim_id == 0 or guest_info.sim_id == traveling_sim.id:
                    continue
        ...
        potential_toddler_caregivers = traveling_sim.sim_info.lives_here or set()
        potential_pet_caregivers = set()
        travel_toddlers_ids = set()
        travel_pet_ids = set()
        for active_sim in services.active_household().instanced_sims_gen():
            if active_sim.id == traveling_sim.id or active_sim.sim_info.id in traveling_sim_ids:
                continue
