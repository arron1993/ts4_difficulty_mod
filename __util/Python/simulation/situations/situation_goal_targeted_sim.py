...
# Compiled at: 2020-09-02 18:04:28
# Size of source mod 2**32: 25034 bytes
import random
    ...
    def _run_goal_completion_tests(self, sim_info, event, resolver):
        if self._target_option == SituationGoalSimTargetingOptions.PlayerChoice:
            if self._situation is not None:
                target = self._select_sims_outside_of_situation or self._get_target_sim_info_from_resolver(resolver)
                if target is None:
                    return False
        else:
