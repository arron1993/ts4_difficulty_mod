...
# Compiled at: 2020-10-29 18:31:12
# Size of source mod 2**32: 44395 bytes
...
class GroupWaypointSituation(SituationComplexCommon):
    ...
    def route_to_waypoint(self):
        if self._social_group is not None:
            ...
            waypoint_generator = self.solo_waypoint_generator((self.current_waypoint_sequence), (self.waypoint_graph.constraint_radius),
    ...
    def choose_next_waypoint(self):
        if self.current_waypoint is not None:
            ...
            if self._num_visited_waypoints >= self.max_waypoints:
            ...
            if self.current_waypoint in self.ending_waypoints:
                if self._num_visited_waypoints >= self.min_waypoints:
        ...
        while self._next_interaction is None and waypoint_budget > 0:
            if self.current_waypoint is None:
                next_waypoint, _ = self.starting_waypoint.choose((self.waypoint_graph), (self.leader_sim.routing_surface),
                  resolver=resolver)
            else:
                ...
                if connections is None:
                    if self.current_waypoint not in self.ending_waypoints:
                        ...
                        if current_count < self.min_waypoints:
                    ...
                    self.on_completed()
                    return False
                next_waypoint, _ = connections.choose((self.waypoint_graph), (self.leader_sim.routing_surface),
                  resolver=resolver,
                  previous_waypoint=(self.previous_waypoint))
            if next_waypoint is None:
                if self.current_waypoint_sequence:
                    return True
                elif self.current_waypoint not in self.ending_waypoints:
...
lock_instance_tunables(GroupWaypointSituation, exclusivity=(BouncerExclusivityCategory.NEUTRAL),
