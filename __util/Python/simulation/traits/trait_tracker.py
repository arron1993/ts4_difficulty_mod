...
# Compiled at: 2020-10-07 00:26:36
# Size of source mod 2**32: 65737 bytes
import itertools
...
class TraitTracker(AffordanceCacheMixin, SimInfoTracker):
    ...
    def can_add_trait(self, trait):
        if not self._has_valid_lod(trait):
            ...
            if trait.trait_type == TraitType.LIFESTYLE:
                return services.lifestyle_service().lifestyles_enabled or False
        return True
    ...
    def _add_trait(self, trait, from_load=False):
        if not self.can_add_trait(trait):
            ...
            if initial_commodities_modified:
                ...
                elif not trait.buffs_add_on_spawn_only or self._sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS):
                    try:
        ...
        if not from_load:
            ...
            self._sim_info.resend_trait_ids(traits_to_add=[trait.guid64])
            ...
            if trait.trait_statistic is not None:
                trait.trait_statistic.on_trait_added(self._sim_info, trait)
    ...
    def _remove_trait(self, trait):
        if not self.has_trait(trait):
            ...
            if trait.disable_aging:
                self._sim_info.update_age_callbacks()
            self._sim_info.resend_trait_ids(traits_to_remove=[trait.guid64])
            if trait.disable_aging is not None:
                self._sim_info.resend_age_progress_data()
        else:
        ...
        if trait.trait_statistic is not None:
            trait.trait_statistic.on_trait_removed(self._sim_info, trait)
